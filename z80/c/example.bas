...
120 NEXT O
200 RESTORE
201 REM No bitwise operations...
205 FOR U=1 TO 7
210 FOR K=8 TO 0 STEP -1
230 IF 2^K<S(U) THEN 270
240 ar(U,9-K)=0
250 GOTO 280
270 ar(U,9-K)=1: S(U)=S(U)-2^K
271 if s(u)= 1 then ar(U,8) = 1
280 NEXT K
290 NEXT U



450 for B = 8 to 0 step -1
451 for k = 0 to 8
455 IF ar(k,b)=0 THEN goto 470
460 out 44,200: goto 480
470 out 44,0
480 next k
490 next b
491 goto 87 
